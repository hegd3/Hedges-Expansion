...
public class GraffModel extends HEModel<GraffEntity> {
	...
	public static LayerDefinition createBodyLayer() {
		...
		PartDefinition neckrot = neck.addOrReplaceChild("neckrot", CubeListBuilder.create().texOffs(0, 70).addBox(-2.5F, -19.0F, -2.0F, 5.0F, 23.0F, 6.0F, new CubeDeformation(0.0F))
				.texOffs(29, 71).addBox(-5.5F, -19.0F, -3.0F, 11.0F, 8.0F, 8.0F, new CubeDeformation(0.01F)), PartPose.offset(0.0F, 0.0F, 0.0F));
	...
	public void setupAnim(GraffEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.animate(entity.callAnimationState, GraffAnimation.call, ageInTicks, 1f);
		if (entity.getDeltaMovement().horizontalDistanceSqr() > 0.01) {
			this.animateWalk(GraffAnimation.run, limbSwing, limbSwingAmount / 2, 1.5f, 2.5f);
		} else {
