...
public class GildGliderEntity extends HEBucketableSchoolingMob {
    ...
    public ItemStack getBucketItemStack() {
        return new ItemStack(HEItems.GILD_GLIDER_BUCKET.get());
    }
        ...
        if ((pReason == MobSpawnType.CHUNK_GENERATION || pReason == MobSpawnType.NATURAL)) {
            ...
            if (groupSize > 0 && !this.level().isClientSide()) {
                for (int i = 0; i < groupSize; i++) {
                    ...
                    GildGliderEntity entity = new GildGliderEntity(HEEntities.GILD_GLIDER.get(), this.level());
